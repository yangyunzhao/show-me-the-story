...
A self-contained AI tool for writing full-length novels. A single executable with a built-in web UI: point it at any OpenAI-compatible API (OpenAI, DeepSeek, a local Ollama / LM Studio, etc.) or a locally logged-in Codex CLI, and it will generate a full outline and write the novel chapter by chapter.
...
On first launch you will be asked to create a project. Once a project is open, choose a model provider on the "Config" page:

- **OpenAI Compatible API**: enter the API base URL, e.g. `https://api.deepseek.com` or `http://localhost:11434/v1` (the trailing `/v1` is optional), the model name, e.g. `deepseek-chat` or `gpt-4o`, and an API key. Local models may leave the key empty.
- **Codex Subscription**: uses the local `codex` command and the Codex account already logged in on this machine. Run `codex login` in a terminal first. The app does not read or save Codex tokens and does not need an OpenAI API key. Enter the Codex model name; leave the Codex working directory empty to use the safe default, or choose an empty/dedicated directory. Do not use the source tree, your home directory, or a filesystem root.

When `codex_use_streaming` is enabled, Codex mode can stream chapter output. When it is off, one-shot tasks still work, but direct chapter streaming is unavailable. Provider configuration is shared across all projects.

Codex troubleshooting:

- `codex` not found: install the Codex CLI and make sure the terminal that starts this app can find `codex` in `PATH`.
- Login or authentication failure: run `codex login` in a terminal, then test the connection again.
- Model unavailable: check that the `Codex model` value is available to the logged-in account.
- If the Codex CLI is logged in with a ChatGPT account, do not assume `gpt-5-codex` is available. Run `codex doctor` and prefer the default `model` shown under `Configuration`.
- Unsafe working directory: choose an empty or dedicated directory. Do not use the source tree, your home directory, or a filesystem root.
- Codex connection tests wait up to 90 seconds. Start, success, failure, and timeout details are written to the terminal and the in-app log panel.
- A successful connection test returns only the model name and response character count. It does not return a model-output sample, so full responses are not exposed in the page or logs.
...
```
<data dir>/
├── api.json                 # model provider / API configuration (shared by all projects)
└── storys/
    └── <project name>/
...
- Transient network errors retry automatically (with exponential backoff). Fatal errors such as an invalid API key, invalid Codex configuration, or an unavailable Codex CLI stop immediately with a message.
